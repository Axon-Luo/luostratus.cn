...
这三个版本与前面不同——它们不是本地独立运行的整合包，而是**为连接特定多人服务器**准备的客户端配置。文件夹内没有本地 mods，模组由服务端在连接时自动分发和加载。

### 云陌挽 CloudCrescent

```
基座版本：1.21.11-Fabric 0.18.4
启动目录：E:\Minecraft我的世界\整合包\26.2我的世界\.minecraft\
```

**云陌挽**（Cloud Crescent）是一个中文 Minecraft 服务器，这个 PCL 配置专为连接该服务器而设。基于 Fabric 1.21.11，指向独立的启动文件夹。

服务器的具体 Mod 列表由服务端控制，客户端无需预先安装——这是 Fabric 服务器常见的 **模组自动下发** 机制，首次连接时自动从服务器下载所需模组。

### MineKU

```
基座版本：XPlus PerioTable based on Minecraft 1.21.11 (Fabric)
分配内存：25GB
界面模式：自定义 UI 类型 3 / 预设 2
```

**XPlus PerioTable**（元素周期表）是一个以化学和科技为主题的 Minecraft Fabric 整合包。MineKU 服务器在此基础上添加了自定义内容，作为一个服务器客户端配置存在。

25GB 的自定义内存分配（远超 Minecraft 默认的 2GB）说明这个服务器整合包规模相当大——通常包含大量科技模组、多维度内容和精细渲染需求。XPlus 系列在中文 MC 科技玩家中颇有知名度，以化学工艺、元素合成和自动化流程为核心玩法。
...
最简配置的服务器入口，"狐"可能是服务器名称或某位朋友分享的自定义包名。保持最小化配置意味着这个版本依赖服务端的完整模组分发，本地不需要预装任何模组。

这三个版本体现了 PCL 启动器的一个重要特性：**多身份隔离**。你可以在不同的服务器客户端之间切换，每个都有独立的配置、UI 和启动参数，互不影响。对于同时玩多个服务器的玩家来说，这种隔离非常重要——不同服务器的模组集合往往互不兼容，混在一起会导致崩溃。
