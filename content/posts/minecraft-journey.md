...
截至 2026 年 7 月，我的 PCL 启动器中已安装 **23 个版本**，涵盖 Fabric、Forge、NeoForge 三大模组加载器，版本跨度从 1.19.2 到最新的 26.2。下面是逐一拆解。
...
这三个版本是为连接特定多人服务器准备的客户端，各有完整的本地模组集合——与前面本地整合包不同的是，它们的模组选择由服务器玩法决定。

### 云陌挽 CloudCrescent（38 个模组）

```
MC 版本：26.2 | 加载器：Fabric
核心关键词：建筑、生电、语音
```

云陌挽是一个以**建筑和红石科技**为核心的 Fabric 服务器。

**建筑与工具链：**

| 模组 | 功能 |
|------|------|
| **WorldEdit** 7.4.4 | 创世神——服务器端大型建造的核心工具 |
| **WorldEdit CUI** 26.2 | 创世神客户端接口——显示选区范围的可视化辅助 |
| **Litematica** 0.28.0/0.28.1 | 建筑投影（两个版本共存） |
| **MaLiLib** 0.29.1 | Litematica / MiniHUD / Tweakeroo 的前置库 |
| **MiniHUD** 0.40.1 | 迷你 HUD——屏幕上显示各种技术信息（光照等级、结构边界等） |
| **Tweakeroo** 0.29.1 | 微调工具集——灵活放置、自动切换工具等 |
| **TweakerMore** 3.32.0 | Tweakeroo 的增强扩展 |
| **Item Scroller** 0.32.0 | 物品滚轮——批量移动物品、一键合成 |
| **Peek** 1.5.1 | 潜影盒内容预览（无需放置即可查看内部） |
| **Shulker Box Tooltip** 5.3.1 | 潜影盒提示框——悬停显示完整内容 |
...
| **Sodium** 0.9.0 | 渲染优化 |
| **Iris** 1.11.1 | 光影 |
| **Sodium Extra** 0.8.9 | Sodium 功能扩展 |
| **Lithium** 0.25.0 | 服务端逻辑优化 |
| **Continuity** 3.0.1 | 连接纹理 |
| **Entity Culling** 1.10.5 | 实体面剔除 |
| **Polytone** 5.10.2 | 纹理/粒子/颜色综合优化 |
| **Entity Model Features** 3.2.5 | 实体模型变体 |
| **Entity Texture Features** 7.1.1 | 实体纹理变体 |

**特色模组：**

| 模组 | 功能 |
|------|------|
| **Simple Voice Chat** 2.6.20 | 游戏内语音通话——生电/建筑服务器标配 |
| **AllMusic Client** 4.0.2 | 服务器背景音乐播放器 |
| **Flashback** 0.41.1 | 游戏内录像回放 |
| **IMBlocker** 7.1.7 | 输入法冲突修复——中文玩家必备 |
| **Inventory Management** 2.0.1 | 背包管理增强 |
| **Chat Heads** 1.2.4 | 聊天头像 |

Voice Chat + WorldEdit + Litematica 的组合说明这是一个**协作建造型**服务器：玩家可以语音沟通设计方案，用投影显示蓝图，用创世神批量操作。
...
```
MC 版本：26.1.2 | 加载器：Fabric
分配内存：25GB | 整合包名称：MineKU官方整合包26.1.2
```

MineKU 是三组中模组数最多的服务器客户端（61 个），基于 26.1.2，内存分配高达 25GB。
...
| **Sodium** 0.8.12 | 渲染优化 |
| **Iris** 1.10.9 | 光影 |
| **Sodium Extra** 0.8.7 | Sodium 扩展 |
| **Lithium** 0.24.6 | 服务端逻辑优化 |
| **FerriteCore** 9.0.0 | 内存优化 |
| **ModernFix** 5.27.17 | 综合性能修复——Minecraft 加载优化 |
| **ImmediatelyFast** 1.15.2 | 即时渲染 |
| **More Culling** 1.7.2 | 更多面剔除 |
| **Entity Culling** 1.10.5 | 实体面剔除 |
| **Bobby** 5.2.13 | 超视距区块缓存——看几十公里外的建筑 |
| **Scalable Lux** 0.2.0 | 可缩放光照引擎 |
| **LambDynamicLights** 4.11.0 | 动态光源——手持火把照亮四周 |
| **Continuity** 3.0.1 | 连接纹理 |
| **Skyboxify** 3.0 | 自定义天空盒 |

**建筑与技术工具：**

| 模组 | 功能 |
|------|------|
| **Litematica** 0.27.8 | 建筑投影 |
| **Syncmatica** 0.3.18 | 共享投影——多人同步同一个蓝图 |
| **MiniHUD** 0.39.7 | 迷你 HUD |
| **Tweakeroo** 0.28.6 | 微调工具集 |
| **TweakerMore** 3.30.0 | Tweaker 增强 |
| **Peek** 1.5.1 | 潜影盒预览 |
| **BBE** 1.3.4 | 方块实体增强——箱子、床等可视化细节 |
| **WTHIT** 19.0.1 | 准星指向信息（What The Hell Is That） |
| **Glow Tools** 1.0.0 | 发光工具显示 |

**体验增强：**

| 模组 | 功能 |
|------|------|
| **Controlify** 3.0.2 | 手柄支持——用控制器玩 Minecraft |
| **Not Enough Animations** 1.12.4 | 更好的第一/三人称动画 |
| **3D Skin Layers** 1.11.2 | 3D 皮肤层——皮肤第二层立体化 |
| **AllMusic Client** 4.0.3 | 服务器音乐系统 |
| **OK Zoomer** 17.0.0 | 望远镜缩放 |
| **Caxton** 1.0.0 | 服务器文字渲染增强 |
| **Language Reload** 1.7.7 | 语言热重载 |

**服务器相关：**

| 模组 | 功能 |
|------|------|
| **Server Pinger Fixer** 1.1.0 | 服务器列表修复 |
| **Packet Fixer** 3.3.5 | 网络包优化——减少掉线和延迟 |
| **No Chat Reports** 2.19.0 | 禁用聊天举报 |
| **Structure Layout Optimizer** 1.1.4 | 结构布局优化 |

25GB 内存 + Syncmatica（共享投影）+ Bobby（超视距）的组合揭示了这个服务器的核心玩法：**多人协作的大型建设项目**。Syncmatica 允许多人同时查看同一份建筑蓝图并分工建造，Bobby 则让远处的建筑成果可见。

---

### 1.21.11狐 — 狐族物语（14 个模组）

```
MC 版本：1.21.11 | 加载器：Fabric
整合包名称：1.21.11-Fabric狐族物语客户端公测正式版
```

这是三组中最轻量的服务器客户端——仅 14 个模组。

**核心模组：**

| 模组 | 功能 |
|------|------|
| **YDM 的武器大师** 4.4.4 | ★ 武器大师——添加特殊武器、战斗技能和连击系统 |
| **EuphoriaPatcher** 1.9.3-r5.8.1 | 综合补丁器——光影兼容、渲染修复、性能优化 |
| **Sodium** 0.8.12 | 渲染优化 |
| **Iris** 1.10.7 | 光影 |
| **Lithium** 0.21.4 | 服务端逻辑优化 |
| **FerriteCore** 8.2.0 | 内存优化 |
| **Entity Culling** 1.10.2 | 实体面剔除 |
| **Continuity** 3.0.1 | 连接纹理 |
| **HMI** 5.1.1 | 手持动画 / 辅助信息面板 |
| **Mod Menu** 17.0.0 | 模组菜单（游戏内管理配置） |
| **AppleSkin** 3.0.8 | 食物饱食度信息 |
| **Mouse Tweaks** 2.30 | 鼠标手势 |
| **Placeholder API** 2.8.2 | 文本占位符前置 |

"狐族物语"这个名字 + YDM 武器大师模组说明这是一个**战斗/RPG 向**的服务器。"公测正式版"意味着这个包仍在积极开发中。武器大师提供了远超原版的战斗体验——新武器类型、组合技和特殊技能。

---

这三个服务器版本体现了 PCL 启动器的**多身份隔离**能力：云陌挽（建筑协作）、MineKU（大型建设）、狐族物语（RPG 战斗）——三个完全不同的服务器，三套完全独立的客户端配置，互不干扰。
...
23 个版本，3,000+ 个模组实例——从原版的钠优化到 Create 的机械帝国，从丧尸末日的硬核求生到航空学的飞天梦想，从单机生存到多人服务器。Minecraft 的无限可能，就藏在这些文件夹里。

> ⛏️ 当前活跃：1.20.1-Forge（生存主档 + 龙之崛起）、机械动力 1.21.1-NeoForge（科技档）、云陌挽 / MineKU（多人服务器）
